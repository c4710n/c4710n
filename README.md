:metal: Hello, There!

- I am a pragmatic programmer.
- I don't chase new technologies blindly.
- I like [Elixir](https://elixir-lang.org/), [Nix](https://nixos.org/) and [Emacs](https://www.gnu.org/software/emacs/).
- I [contribute to open source projects](https://github.com/pulls?q=is%3Apr+author%3Ac4710n) (need login to view).

## The Career Plan

Elixir makes me feel settled, and I will spend my next decade(2021-2030) on Elixir and Erlang/OTP.

From 2019, I started to use a top-down approach to learning Elixir in my spare time.

- Learn basic Elixir
  - [x] Read [Elixir School](https://elixirschool.com/en/), understand basic syntax
- Learn to write traditional MVC application
  - [x] [Programming Phoenix](https://pragprog.com/titles/phoenix14/programming-phoenix-1-4/) - Part 1
  - [x] [Programming Ecto](https://pragprog.com/titles/wmecto/programming-ecto/) - Part 1
- Learn to write real-time application
  - [ ] [Programming LiveView](https://pragprog.com/titles/liveview/programming-phoenix-liveview/)
  - [ ] Real-Time Phoenix
- Learn Elixir in depth
  - [ ] Elixir in Action
- Learn to design Elixir Systems with OTP
  - [ ] The Little Elixir OTP Guidebook
  - [ ] Designing Elixir Systems with OTP
- More
  - [ ] Learn Functional Programming with Elixir
  - [ ] Metaprogramming Elixir
  - [ ] Programming Erlang
  - [ ] Erlang in Anger

> - [x] BOOKS READ
> - [ ] BOOKS BEING READ / PLAN TO READ

## The Good, Old and Confused Time

### 2017.09 ~ 2020.12 | Frontend Developer

Wrote marketing web pages with vanilla JavaScript, [React](https://reactjs.org/), [BLACKSMITH 2D](https://blacksmith2d.io/) and [PixiJS](https://www.pixijs.com/) at [YIZ](https://yiz.design/).

Projects created with my own framework - [ft](https://github.com/yiiz/ft):

- [来跑跑寻找你的 Line Friends 伙伴](https://demo.yiz.design/qq-popcart-linefriends-perf/index_master.html)
- [王者荣耀 2019 年中秋节小游戏](https://demo.yiz.design/kog-maf-2019-rmrb/)
- [LOL 电玩研究所](https://demo.yiz.design/lol-video-game/) (FTG scene is developed by [cm](https://github.com/cm-MMK-2))
- [LOL 冰雪节 2018 主题活动](https://demo.yiz.design/lol-snowing-festival-2018/)
- ...

### 2017.03 ~ 2017.09 | Backend Developer

Learn to be a backend developer writing [Node.js](https://nodejs.org/). Struggled in its immature ecology.

### 2016.04 ~ 2017.02 | Traveler

Traveled around China. Learn [Node.js](https://nodejs.org/) and Web Technologies. Tried to be an indie hacker.

### 2014.08 ~ 2016.03 | DevOps

Most of the time, I played with [Docker](https://www.docker.com/), wrote automation tools with [Python](https://www.python.org/) and [Node.js](https://nodejs.org/).
